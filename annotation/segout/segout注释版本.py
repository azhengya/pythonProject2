...
class AnalysisWatchImg:
    ...
    def __init__(self, path, class_name_json, img_size ,model_file):
        ...
        self.gan = 1(, 2, 8, 9) # 前面的1是干什么的，干垃圾中的类在class_name.json中对应的键
        self.shi = (3, 4, 5, 6, 7, 10, 11)
        self.page = 59 # 每次只能计算60个，可以更改

    def get_files(self): #取图片，将图片名字放到列表中
        file_path_list = []
        class_name_list = []
    ...
    def imgs_predic(self,url_list): #识别图片，进行分类
        lengh = len(url_list)
    ...
    def get_autocontrast_confidence(self, inputs, class_name_list): # 判断是干垃圾还是湿垃圾
        '''
        autocontrast_info = {
    ...
    def plt_img(self, imgs_class_name, imgs, intput_array, ds_class_name, info): # 将运行结果输出
        fig = None
        img_name = None
